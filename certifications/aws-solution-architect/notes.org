#+title: AWS Solution Architect

References:

[[https://www.youtube.com/watch?v=Ia-UEYYR44s][Free Code Camp]] tutorial
* Table of Content
1. +[[S3 (Object Storage)]]
2. +[[Snowball]]
3. +[[Virtual Private Cloud (VPC)]]
* AWS Certified Solutions Architect

** About the cert + exam
Valid 3 years, passing score 72%
To book an[[https://aws.amazon.com/certification/?nc2=sb_ce_co][exam]]

** S3 (Object Storage)
+ [[Table of Content]]
*** Intro
S3 stands for Simple Storage Services
Typically suitable for very large volumes of static data—especially unstructured data. Objects contains your data, they are like files (images, CVS, etc) ranging from 0 Bytes to 5 TB.
An S3 Bucket contains all of the objects.

*** Storage Classes
In order of costs and availability:
- S3 Standard
- S3 Intelligent-Tiering
- S3 Standard-IA (infrequent access)
  Recomended if you access the files less than once a month (retrival fees otherwise)
- S3 One Zone-IA
  Same a above but objects only exists in a single [[https://aws.amazon.com/about-aws/global-infrastructure/regions_az/][availability zone]] so reduced durability
- Glacier
  For data that is maybe used 1-2 a year
- Glacier Deep Archive
  Retrival times of 12 hours (stored on magnetic tape) recommended for data records (7-10 years)
  [[https://aws.amazon.com/s3/storage-classes/][References]]

*** Security
**** S3 policies
S3 bucket policies specify what actions are allowed or denied for which principals on the bucket that the bucket policy is attached to (e.g. allow user Alice to PUT but not DELETE objects in the bucket). TLDR Control access in S3 environment
#+begin_src json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": ["arn:aws:iam::111122223333:user/Alice",
                "arn:aws:iam::111122223333:root"]
      },
      "Action": "s3:*",
      "Resource": ["arn:aws:s3:::my_bucket",
                   "arn:aws:s3:::my_bucket/*"]
    }
  ]
#+end_src

**** S3 Access Control Lists
S3 ACLs is a legacy access control mechanism that predates IAM. An S3 ACL is a sub-resource that’s attached to every S3 bucket and object. It defines which AWS accounts or groups are granted access and the type of access. When you create a bucket or an object, Amazon S3 creates a default ACL that grants the resource owner full control over the resource.

**** S3 permissions
The following example bucket policy grants the s3:PutObject and the s3:PutObjectAcl permissions to a user (Dave). If you remove the Principal element, you can attach the policy to a user.
#+begin_src json options
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Sid": "statement1",
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::12345678901:user/Dave"
            },
            "Action": [
            "s3:PutObject",
            "s3:PutObjectAcl"
            ],
            "Resource": "arn:aws:s3:::awsexamplebucket1/*"
        }
    ]
}
#+end_src json
The following policy, when attached to a role/user allows full access to all s3 bucket in the account. If you need to access another account use `sts:AssumeRole`
#+begin_src json options
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Action": [
                "s3:*",
                "s3-object-lambda:*"
            ],
            "Resource": "*"
        }
    ]
}
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Sid": "VisualEditor0",
            "Effect": "Allow",
            "Action": "sts:AssumeRole",
            "Resource": "arn:aws:iam::{number}:role/{role-name}"
        }
    ]
}
#+end_src json
*** S3 Encryption
Uses AES-256 by default
- Encryption in transit (SSL/TLS)
- Server side encryption (SSE)
  * SSE-AES: S3 handles the key and uses AES-256
- Client side encryption

*** S3 Data Consistency
- New Objects (puts)
new S3 Objects can be read immediately after writting
- Overwrte (PUTS)/Delete Objects (DELETE)
  When you overwrite or delete an object it takes time for S3 to replicate versions to AZs

*** S3 Cross Region Replication (CRR)
Any Object will be automatically replicated to another region.
Must be enabled alongside versioning for both the source and destination. Works for multi-account.

*** S3 Versioning
Store all versions of an object in S3 ( cannot be disabled once activated ) kinda like Git
Every new object uploaded will be given a hversion ID unless versioning was off when they existed prior versioning

*** S3 Lifecycle Management
Automate the process of moving objects to different storage classes. Can be used with versioning. Usefull when data must be kept for record keeping.

*** S3 Transfer Acceleration
To transfer large files over long distances, users uses a distinct URL for an Edge location. Using AWS Backbone Network the edge location transfers to the S3 bucket.

*** S3 Presigned Urls
Generates a url which provides you temporary access to an object to either upload or download object data
`aws s3 presing s3://mybucket/myobject --expires-in 300`

*** S3 MFA delete
Multi Factor Authentification (MFA) which prevents users from deleting objects unless they provided their MFA code. Requires the following:
- AWS CLI must be used
- the bucket must have versioning turned on
#+begin_src bash options
aws s3api put-bucket-versioning \
--bucket bucketname \
--versioning-configuration Status=Enabled,MFADelete=Enabled \
--mfa "your mfa serical number "
#+end_src bash
Only the bucket owner can DELETE
*** TODO Monitoring (clouflair)
*** TODO Terraform
*** Misc
If Public and versioning is enabled, any new files version of an object will be set to private

*** S3 Follow along
[[https://youtu.be/Ia-UEYYR44s?t=1926][video]]

** Snowball
+ [[Table of Content]]
Petabyte transfer service (move data to AWS using a briefcase computer)
Essentially instead of transfering 50TB+ data over high speed internet we do so using that briefcase.
*** Snowball edge
Same with snowball but more storage and compute power (100TB) and can act as edge nodes. Can be clustered and specialized for storage/compute/GPU optimized
*** Snowmobile
Essentially AWS ships you a massive container w/ 100 PB

** Virtual Private Cloud (VPC)
+ [[Table of Content]]
Can be thought as your own personal data centre
#+attr_html: :alt  :align left :class img :width 500px
[[./images/aws-vpc.png][vpc general overview]]
*** Key features
VPC are region specific and you can have up to 5 per region
a maximum of 200 subnets per VPC are possible

*** About CIDR
Classless Inter-Domain Routing (CIDR) blocks are for specifying a range to IP addresses in format of IPv4 or IPv6
x, y, z and t are numbers from 0 to 255. Basically, each represents an 8 bit binary number. That's why it is range is up to 255. Combination of this numbers becomes an IPv4 IP address that must be unique to be able to identify a specific instance.

In case of AWS, p is a number from 16 to 28. It represents the number of bits that are inherited from given IP address. For example: 10.0.0.0/16 represents an IP address in following format: 10.0.x.y where x and y are any number from 0 to 255. So, actually it represents a range of IP addresses, starting from 10.0.0.0 to 10.0.255.255.

A CIDR range of 10.0.0.0/16 refers to all IP addresses that start with 10.0.x.x.

*** Dynamic Host Configuration Protocol (DHCP)

*** Default VPC
AWS has a default VPC in every region
Creates the following:
- Internet Gateway
- Default Security Group
- Default Network Access Control List (NACL)
- Associated default DHCP

*** VPC peering
Connects one VPC to another so that they behave like they are on the same network. You can not use one VPC as an intermediate to another and no overlapping CIDR blocks (check picture)
#+attr_html: :alt  :align left :class img :width 500px
#+caption: how to connect 2 VPC together
[[./images/vpc-peering.png][vpc-peering]]

*** Route Tables
Used to determine where network traffic is directed. Each subnet must be associated with a route-table
of note a record of 0.0.0.0/0 grants public access to everything from anywhere

*** Internet Gateway (IGW)
Does the following:
1. provides a target in your VPC route tables for internet-routable traffic (inside/outside)
2. proivdes Network Address Translation (NAT) for instances which have been assigned public ipv4 traffic

#+attr_html: :alt  :align left :class img :width 500px
#+caption: Diagram of IGW
[[./images/vpc-IGW.png][Internet Gateway]]

*** Bastion/Jumpbox
Bastions are security hardened EC2 instances designed to help you gain access to your EC2 instance in a private subnet.

#+attr_html: :alt  :align left :class img :width 500px
#+caption: How to configure a Bastion host
[[./images/vpc-bastion.png][bastion EC2 instance]]

**** Modern architecture relies on Session Manager
Only requires an IAM policy attached to the role/user
#+begin_src bash options
aws --profile <your-profile> ssm start-session --target i-<ec2-ID>
#+end_src bash
#+attr_html: :alt  :align left :class img :width 500px
#+caption: SSM architecture
[[./images/vpc-session-manager.png][SSM]]
A working example can be found [[https://medium.com/canisworks/aws-systems-manager-vs-bastion-hosts-for-private-networks-efe9a42f5ad7][here]].

*** Direct Connect
AWS solution to establishing dedicated network between on-premises (offices/home) and AWS. This is not the same

*** VPC Endpoints
Allows you to privately connect your VPC to other AWS services without leaving the AWS network.
Eliminates the need for Internet Gateway, NAT device, VPN connection
2 types:
**** interface endpoints
Elastic Network Interfaces (ENI) with a private IP addess. Thye ve as an entry point for traffic going to a supported service. Check AWS PrivateLink. Pay per GB tx
Wide applications (EC2 API)
**** Gateway endpoints
Is a gateway that is a targetfor a specific route in your route table. Free
Only supports S3 and DynamoDB

*** VPC Flowlogs
Allows for the capture of IP traffic information in-and-out of network interfaces within your VPC. Data is stored in CloudWatch logs.
Flow logs can be created for:
- VPC
- Subnets
- Network Interface
Always store the source and destination ip address (exam question)

*** Network Access Control List (NACL)
NACL acts as a virtual firewall at the subnet level. VPC automatically get a default NACL. It can =allow= or =deny= traffic unlike Security Groups which can only allow.
Highest rule # is 32766 but it is recommended to incremement from 0 to 10/100

**** Use case
A malicious actor at a specific IP address (normally they have rotating) is trying to access our instances. So you simply add the IP to the NACL and deny is same with port 22 (SSH) in the event that we will never ssh into the application.

*** Security Groups
You cannot block specific IP address with SG use the NACL instead.
Associated with EC2 instances, each SG contains a set of rules that filter inbound and outbound of EC2 instances. All traffic is blocked by default.
Multiple instances accross subnets can belong to a security group.

You can specify the source to be an IP range e.g 170.168.0.0/16 or source from another security group.
Instances can belong to multiple security groups. For example SG A has nothing written (denys all) and SG B located withing SG A has allow inbound:80/22 then SG A will allow both
**** Limits
Can have up to 10k SG in a region
can have 60 inbound and 60 outbound rules per SG
16 SG per Elastic Network Interface.

*** Network Address Translation (NAT)
Method of re-mapping one IP address space into another. Used to allow private networks outbound access to the internet. Allows for network deconflicting.

**** Instances vs Gateway
Instances (legacy) individual EC2 instances
Gateway is a managed service which launches redundant instances within the selected AZ.


*** VPC Follow along
[[https://www.youtube.com/watch?v=Ia-UEYYR44s&t=5004s][video]]

**** Notes
** Identity Access Management  (IAM)
*** Core components
- IAM users: end users who log into the console or interact with AWS ressources.
- IAM Groups: Group up your users so they all share permission levels: e.g. admin, Dev, Auditors
- IAM roles: Associates permissions to a role and then assign them to an user/group
- IAM policies: JSON documents which grants permissions for specific user/group/role to access service
  policies are attached to an IAM identiy
  Generally you want to attach policies to roles which are then attached to users/groups. Of course a user can use inline policy instead but its not reproductible.

*** Managed vs Customers vs inline Policy
- Managed policies: cannot be edited and are managed by AWS (orange)
- Customer Managed Policies: bread and butter of IAM.
- inline policy: policy directly attached to the user.

*** policy structure
- version: policy language version
- statement: container for hte policy elements can be an array []
- sid: statement identifier (optional) labelling
- effect: either allow or deny
- principal: acct, user , role or federated user
- Resource: which AWS resource is affected
- Condition: circumstances under which the policy grants permission

#+begin_src json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": ["arn:aws:iam::111122223333:user/Alice",
                "arn:aws:iam::111122223333:root"]
      },
      "Action": "s3:*",
      "Resource": ["arn:aws:s3:::my_bucket",
                   "arn:aws:s3:::my_bucket/*"]
    }
  ]
#+end_src
*** Password policy
IAM can set a password policy for a certain password length/char/etc and ask users to rotate every so often

*** Access keys
Allows users to interact with AWS service programmatically via the AWS CLI or AWS SDK. max 2 per users

*** MFA
CAnnot be enforced directly, but admin can restrict access to resources using policy requiring MFA.

*** Examples

Grants access to one or more specific s3 bucket and can narrow down to the file.
[[https://aws.amazon.com/blogs/security/writing-iam-policies-grant-access-to-user-specific-folders-in-an-amazon-s3-bucket/][References]]

#+begin_src json
{
 "Version":"2012-10-17",
 "Statement": [
   {
     "Sid": "AllowUserToSeeBucketListInTheConsole",
     "Action": ["s3:ListAllMyBuckets", "s3:GetBucketLocation"],
     "Effect": "Allow",
     "Resource": ["arn:aws:s3:::*"]
   },
  {
     "Sid": "AllowRootAndHomeListingOfCompanyBucket",
     "Action": ["s3:ListBucket"],
     "Effect": "Allow",
     "Resource": ["arn:aws:s3:::my-company"],
     "Condition":{"StringEquals":{"s3:prefix":["","home/", "home/David"],"s3:delimiter":["/"]}}
    },
   {
     "Sid": "AllowListingOfUserFolder",
     "Action": ["s3:ListBucket"],
     "Effect": "Allow",
     "Resource": ["arn:aws:s3:::my-company"],
     "Condition":{"StringLike":{"s3:prefix":["home/David/*"]}}
   },
   {
     "Sid": "AllowAllS3ActionsInUserFolder",
     "Effect": "Allow",
     "Action": ["s3:*"],
     "Resource": ["arn:aws:s3:::my-company/home/David/*"]
   }
 ]
}
#+end_src

** Cognito
Decentralized managed Authentification system for mobile + desktop apps.
#+attr_html: :alt  :align left :class img :width 500px
#+caption: cognito
[[./images/cognito.png][Cognito overview]]

*** Web Identity Federation andd Identity Provider (IdP)

**** Web Identity Federation
To exchange identiy and security information betweenan identity provider and an application.

**** Identiy Provider (IdP)
A trusted provider of your user identity taht lets you user their authentication to accesss other services. e.g. FB, Github, Google, etc.

uses SAML

*** Cognito Users Pools
User pools are user directories used to managed action for web/mobile apps such as sign-up/sign-in/acct recovery/ etc.
Successful identification generates a JWT token

*** Cognito Identity Pools
Provides temporary (with JWT) credentials to access services e.g. S3

*** Cognito Sync
sync user data and preferences across devices. Uses push synchronization. Uses Simple Notification Service (SNS) to send notification to all user devices.

** Domain Name System (DNS)
In AWS you can buy + manager a domain name with route53
Translates domain name e.g. github.com into a routable internet Protocol address e.g 138.12.123.01
Can use either IPv4 (32 bits) or IPv6 (128bits)

*** Domain Registrars
Authorities who have the ability to assign domain names under one or more top-lvel domains. Good examples are Namecheap.com/godaddy.com/AWS/etc.
Can use WhoIS database to see what the domain name leads to

*** Top level domains
example.co(top-lvl).uk (Second-lvl)
Top-level domains names are controlled by the internet Assigned Numbers Authority (IANA)
[[https://www.iana.org/domains/root/db][to query]]

*** Start of Authority (SOA)
Every domain must have a n SOA record. The SOA is a way for the Domain Admins to provide information about domain.

*** Address Records
Convert name of domain to an IP address

*** Canonical Names (CNAME)
Resolves one domain name to another rather than an IP address. Main advantage is that they are unlikely to change while IP addresses do.

*** Name server (NS)
Used by top-level domain servers to direct traffic to the DNS server containing the authoritztive DNS record. e.g. Route 53 would have 4 NS

*** Time to Live (TTL)
Length of time that a DNS record gets cached. I recommend 5 minutes

** Route53
AWS DNS service to register and manager domains. Think of namecheap but on steroids (especially if your infra is on AWS)
Decentralized managed Authentification system for mobile + desktop apps.

#+attr_html: :alt  :align left :class img :width 500px
#+caption: example complex Route53
[[./images/route53.png][Cognito overview]]

*** Alias record
routes traffic to specific AWS resources. Detects the change in IP address and ensure that its connected.

*** Routing policies
Best viewed through AWS =traffic flow=
**** Simple routing
default, multiple addresses result in random selection
e.g. the record =random.vandersleyen.dev= is tied to 3 different IP addresses
**** Weighted routing
traffic based on weighted values to split traffic
**** Latency-based routing
traffic to region resource with lowest latency
**** Failoever Routing
route traffic if primary endpoint is unhealthy
**** Geolocation Routing
traffic based on the location of the user
**** Geo-proximity Routing
Traffic based on the location of your resources.
**** Multi-value Answer Routing
respond to DNS queries with up to 8 healthy records

*** Health checks
Essentially like uptime-kuma but for AWS. every 30sec (10s min),

*** Route53 resolver
A regional service that lets you route DNS queries between your VPCs and your network.
Usefull for hybrid environments.

** Elastic Compute Cloud (EC2)
EC2 is a resizable compute capacity where you can configure your os, instance type, storage and more (IAM roles, Security groups, etc.)

*** Instances types
[[https://aws.amazon.com/ec2/instance-types/][references]]
- General purpose
  Balance of compute, memory and networking
- Compute optimized
  Ideal for compute bound applications that benefit from high performnace processor. dedicated gaming servers
- Memory optimized
  Fas performance for workload that process large data sets e.g. caches
- Accelerated optimized
  hardware accelerators/co-processors. e.g. ML
- Storage optimized
  high, sequential read adn write access to very large data sets on local storage. e.g. NoSQoL

*** Instance Profile
holds a reference to a role. AWS will automatically create the instance profile for you.
Always avoid embedding your AWS credentials when possible. Attach a role to an instance via an instance Profile

*** Placement Groups
Let oyu choose the logical placement of your instances to optimize communication, performance or durability. Its free.
- Cluster
  packas instances close together inside an AZ
- Partition
  spread instances across logical partitions. Does not share individual hardware (different racks)
- Spread
  Each instance is placed on a different rack. Max of 7 can be spread across multiple AZ

*** UserData
its a scrip\t that will be automatically ran when launching/creating an EC2 instance. e.g. install a few packages
Can be accessed

*** Metadata
From within your EC2 instance can access information about the EC2 instance via a special url endpoint at.
169.254.169.254/latest/meta-data
/public-ipv4 get current ipv4 address
/ami-id the AMI ID used to launch this EC2 instance
/instance-type the instance type of EC2 instance

*** EC2 Pricing
- On-demand:
  pay per hours, low costs and flexible. Usefull for short and spiky loads
- Reserved:
  commit to EC2 over a 1 - 3 year term. Can resell unused reserved instances.
  Can be paid all upfront, partial or no upfront (the more upfront the greater the rebate)
  Offering classes:
  * std, upto 75% reduced pricing but cannot change instances attributes (e.g. size)
  * convertible up to 54%. Allows you to change RI attributes if greater or equal in value
  * Scheduled, reserve the instances for specific time periods e.g. once a week for a few hours
- Dedicated:
  Dedicated servers, can be on-demand or reserved. Usefull for when you have to meet regulatory requirements
  * multi-tenant: multiple customers are running workloads on the same hardware (virtual isolation)
  * single-tenant: physical isolation of the hardware e.g. I have the entire rack DEDICATED to me
- spot:
  up to 90% off
  Requests spare computing capacity. Frequent starts and stops.
  Usefull for instances that have flexible start and end times.
  Use =AWS Batch= for spot pricing
  if AWS terminates your instances the partial hour is free

** Amazon Machine Image (AMI)
A template to configure new instances. You can turn your EC2 instances into AMIs so you can create a copy of it. Region specific.
Generaly used with =AWS System Manager Automation= and =Auto hscaling Groups=
*** Marketplace
You can use 3rd party vendor blueprint for it

*** Creation
Can create an AMI from an existing EC2 taht is either running or stopped

*** Choosing
AWS has hundreds of AMIs you can use:
- Community AMI are free AMIs maintained by the community
- AWS Marketplace are free or paid AMIs maintaned by vendors
Since AMIs are region specific due to regulatory requirements, they will have different AMI ID per region.
They can be backed by either:
- Instance Store
- EBS Backed volumes (preserves the data through instances stops and terminations and can be backed with EBS Snapshots)

** Auto scaling Groups
Set scaling rules which will automatically launch additional EC2 instances or shutdown instances. Contains a collection of EC2 instances taht are treated as a group.
Automatic scalling occurs via the following factors:
- Capacity settings
  set min/max and desired capacity. Will always launch instances to meet minimum capacity
- Health check replacements
  EC2 status check
  ELB checks by pinging an https endpoint with an expected response e.g. uptime-kuma
- Scaling policies
  You can set the target
  based on metric e.g. CPU load, network in/out
  * Scaling policies with steps
    scales when an alarm is breached and escalates based on the alarm

*** Use case
#+attr_html: :alt  :align left :class img :width 500px
#+caption: ASG
[[./images/ASG.png][ASG]]

*** Launch configuration
Is an instance configuration template that an ASG uses to launch EC2 instances.
Uses AMI templates to do so. the configuration cannot be edited, when you need to update it you create a new/clone it and then manually associated that new config with the ASG.

*** Launch templates
Same as above but with versioning

** Elastic Load Balancer
reference:
- [[https://www.cloudflare.com/learning/ddos/glossary/open-systems-interconnection-model-osi/][OSI Layers]]
Distributes traffic accross multiple targets such as EC2 instances, containers ECR, IP addresses and lambda functions
3 types:
1. Application load balance ALB http/https
   They operate at layer 7 (Application)
  Designed to balance http and https traffic. has a feature called Request routing which allows routing rules.
  Overall great for web apps
2. Network Load Balancer NLB TCP/UDP
   They operate at layer 4 (transport)
   can perform cross zone load balancing.
    Great when network performance is critical.
3. Classic load balancer CLB legacy
   mix between layer 7 and 4
   responds 504 error (timeout) if the underlying application is not responding

*** Sticky session
Advanced load balancing method that allows you to bind a user's session to a specific EC2 instance. Using cookies
Ensure all request from taht session are sent to the same instance.
Can be enabled for ALB but can only set on a target group and not individual EC2 instances
Usefull if  the information is only stored locally on a single isntance.

*** X-forwarded-for (XFF) Header
if you need the IPv4 of a user, check the X-forwarded-For header
common method for identifying the originating IP address

*** ELB Health checks
ELB does not kill unhealthy instances (that's the job of Autoscalling group)
ALB/NLV health check are found on the target group. essentially same as uptime-kuma

*** Cross zone load balancing
Only available for classic and network load balancer. Extra charges may apply when enabled
#+attr_html: :alt  :align left :class img :width 500px
#+caption: ELB cross AZ
[[./images/ELB-cross-zone.png][Cross zone ELB]]

*** Request routing
Apply rules to incoming request and then forward/redirect based on a number of criterias
#+attr_html: :alt  :align left :class img :width 500px
#+caption: request routing
[[./images/ELB-request-routing.png][Request routing]]

** Elastic File System
   - +[[Table of Content]]
*** Introduction
Attach a single file system to multiple EC2 instances without worrying about disk space. Grows and shrinks based on usage (hence the elastic name)
Multiple EC2 instances in the same VPC can mount a single EFS volume (that is in the same VPC) using NFSv4.1 and creates multiple mounting target.

** Elastic Block Store
   - +[[Table of Content]]
*** Introduction
virtual hard drive in the cloud, creates new volumes that attaches to EC2.
- IOPS: input/output per second. speed at which non-contiguous (non-adjoining section) reads and writes are performed.
- throughput
  data transfer rate to and from storage
- bandwidth
  measurement of the total possible speed of data movement along the network.
*** Types of EBS storage
-- SSD
1. General purpose (SSD) 16k iops, best for most workloads
2. provisioned IOPS (SSD) when high input output is required + latency. Excellent for RDS
-- HDD
3. throughput optimized (HDD): magnetic drive optimiEd for quick data transfer
4. Cold HDD: lowest cost for infrequent access workloads
5. EBS magnetic: previous generation HDD
*** drives
- HDD excellent at writting long and continuous amount of data.
- SSD very good at read/writes (I/O) and no physical moving parts
- Magnetic tapes: durables for decades, cheap, high capacity but slow
*** Moving volumes
#+attr_html: :alt  :align left :class img :width 500px
#+caption: moving volumes
[[./images/EBS-move.png][moving EBS]]
*** Encryped root volume
if not done at creation you can do the following:
- take a snapshot of the unencryptted volume
- create a copy of that snapshot and select the encryption option
- create an AMI from the snapshot
- launch new EC2 instance from the created AMIs
*** EBS vs Instance store volumes
EBS:
- durable, block-level storage, where data persist
Instance Store Volumes:
- a temporary storage
- Cannot stop/restart instances only terminates
- data will be lost if EC2 host fails

** Cloudfront
   - +[[Table of Content]]
*** Introduction
Content distribtuion network (CDN) which is a distributed network of servers which delivers pages and content to users based on the geo location. e.g. they can have fast performace without having to live by the data center
*** Core components
#+attr_html: :alt  :align left :class img :width 500px
#+caption: moving volumes
[[./images/cloudfront.png][moving EBS]]
*** distributions
There are 2 types of distributions
- web
- RTMP (streaming media)
Can enable restrictions
create custom error pages
behaviors: rediredt to https, TTLs
*** lambda@edge
you can use lambda functions to override behavior or request and responses
1. viewer requests (when cloudfron receives a request from a viewer)
2. origin request (before cloudfront forwards a request to the origin)
3. viewer response
4. origin response
can be used for cognito for authentication
*** protection
can be enabled using the original identiy accesss (OAI)
signed cookies: cookie that is passed along with the request to cloudfront.
signed urls: (not the same as s3 presigned url) a url which provides temporary access to cached objects

** Relational Database Service (RDS)
   - +[[Table of Content]]
*** Introduction
managed relational dbs supporting multiple SQL engines (e.g. mysql, postgres, oracle,...)
*** Encryption
handled using AWS key management services (KMS)
*** Backups
two solutions:
- automated: retention between 1 and 35 (no additional costs)
- manual: backup that persist even if the original RDS instance is deleted
*** restoring backup
restore db using the transaction log relevant to that day down to the second.
A new RDS instance is created when restoring which will generate a new DNS endpoint.
*** RDS multi AZ
ensure db remains avail if another AZ becomes unavailable
Makes an exact copy of your db in anohter az. Master/slave sync relationship. if master fails, slave will take over as master.
Only one AZ has a master RDS db the rest of the AZ are slaves
highly durable
*** RDS read replicas
create a read only db replicas. max of 5 replicas per RDS.
highly scalable

** Aurora
   - +[[Table of Content]]
*** Introduction
Fully managed postgres/mysql db that scales and tuned to be fast
1/10th cost of other solutions
postgres optimized to be 3times faster than

*** scaling
starts with 10gb and scales in tranches of 10gb up to 64TB.
computing resources can scales up to 32 vCPU and 244 gb ram
*** availability
minimum 3 Availability zone with 2 copies in each.
Lose up to 2 copies without affecting write
Lose up to 3 copies without affecting read
*** Fault tolerance and durability
self-healing storage (continous scans) for errors in the data.
aurora backup + faiovers handled automatically
*** replicas
#+attr_html: :alt  :align left :class img :width 500px
#+caption: aurora replicas
[[./images/aurora.png][replicas]]
*** Serverless
only pay what you consume.
good for blogs or small volumes
** Redshift
   - +[[Table of Content]]
*** Introduction
Fully managed petabyte data warehouse. Prob better and cheaper than running athena queries on s3 buckets (5$/tb)
#+attr_html: :alt  :align left :class img :width 500px
#+caption: databases vs data warehouses
[[./images/redshift.png][redshift]]
tldr: use db for quick and simple transactions focusing on write, warehouse for read accross vast amount of data.

redshift is a columnar storage database
columnar database storage is different from traditional relational db (row based) in which the search instead of being row by row is done column wise (can be any number of collumns)

*** operation
single nodes
up to 160 gb per nodes

multi-nodes
leader node: manage client connections
compute node: default 32 nodes but can ask AWS for up to 128 nodes

There are 2 types of nodes:
- dc: dense compute (high perf, low storage)
- ds: dense storage

redshift compute nodes, uses nodes (up to 160gb) and can have up to 128 nodes attached
*** compression
redshift uses different compression techniques to maximize compression
nneed for indexes or views
*** operation
uses massive Parrallel Processing
*** Backup
default retention period 1 day max 32
always attempts to maintain 3 copies (s3,original,replica on the compute nodes)
*** billing
**** backup
cost of an s3 bucket
**** data transfer
when data is transfered through a VPN
**** compute node hours
the total number of hours ran accross all nodes in the billing period.
not charged for leader node hours
*** security
data-in-transit encrypted using SSL
data at rest using AES-256encryption
Database encryption can be applied using AWS KMS or hcloudHSM
*** Availabillity
redshift are single AZ only. Multi AZ means multi redshift cluster with same input
** DynamoDB
   - +[[Table of Content]]
     *** Introduction
     A key value pair and document database (noSQL) which guarantees consistent reads and write
     All data is stored on SSD storage and is spread accross 3 different regions.
     you can specify the r/w per seconds (affects the price)
*** Table Structure
PKI: 2 columns (partition key + sort key)
item is a row
attribute is a value in a specific row/column
*** Reads
when data needs to be updated it has to write updates to all copies which can lead to inconsistencies. 2 read consistencies to meet your needs:
- eventual consistent reads: fast reads but can be inconsistent as they aren't all updated at the same time
- strongly consistent reads: will not return a results until all copies are consistent. high latency
*** Write

** Cloudformation (AWS terraform)
   - +[[Table of Content]]
*** Introduction
AWS own infrastructure as code (IaC)
Can use either json/yaml files
*** template
#+attr_html: :alt  :align left :class img :width 500px
#+caption: template
[[./images/cloudformation.png][AWS IaC]]

** Cloudwatch
   - +[[Table of Content]]
*** Introduction
Monitoring services for AWS resources (AWS prometheus)
- clouwatch Logs
- cloudwatch metrics
- cloudwatch events
- cloudwatch alarms
- cloudwatch dashboard

*** Cloudwatch logs
A log Group is a collection of logs. logs files must belong t a log group
A log in a log groups is called a logstream
logs never expires
p.s. AWS services are integrated with cloudwatch logs

*** Cloudwatch metrics
graph your resources predefined metrics

*** Cloudwatch events
event source (e.g. cpu usage) -> targets (what to trigger)

*** Custom metrics and high resolution
using AWS CLI or SDK you can create and publish your own custome metrics. metrics can either be std resolution or high resolution.
High resolution can be down to a second

*** Cloudwatch alarms
most usefull ( billing alarms )
trigger a notification based on metrics.

*** Cloudwatch dashboard
create your own dashboard like prometheus/grafana

*** Cloudwatch availability of data
basic monitoring: ec2 5 minutes, other 1 minute
defailed monitoring (cost money) ec2 1 minute, nil change

*** Agent and host level metrics
Some metrics requires a cloudwatch agent. An agent is a script that can be installed via =System manager=. Run command to target ec2 instance.
e.g. Disk space/memory usage
default e.g. CPU usage, network usage, disk usage

** CloudTrail
   - +[[Table of Content]]
*** Introduction
service that enables governance, compliance, operational auditing and risk auditing.
Used to monitor API calls and actions made on an AWS account.
Easily identiy which users and account made the call to AWS e.g.
- originating IP
- when
- who/ UserAgent
- what region, resource

*** Event history
std log retention 90 days
if longer is required you must create a trail which are outputted to S3. To query the trail you need to use Amazon Athena.

*** options
- A trail can be set to log to all regions
- accross all ccounts in an organization
- You can encrypt your logs
- to ensure logs integrity and to see if they were tampered with we have to turn on log file validation.

*** Cloudtrail to cloudwatch
Can be sent to cloudwatch. Cannot do the other way around

*** Management events vs data events
**** Management events (90%)
track management operation (turned on by default and can't be turned off)
- config security
- registering devices
- setting up logging

**** Data events (10%)
track specific operation for specific AWS services. Only 2 services:
- s3 (putObject, read, etc.)
- ec2
** AWS Lambda
   - +[[Table of Content]]
*** Introduction
run code without provisioning or managing servers. Serverless functions, pay per invocation.
Natively supprot 7 runtime languages:
- ruby, python, java, go, powershell, nodejs, c#
  you can create own runtime events
Only pay for the compute time consumed.

*** use cases
#+attr_html: :alt  :align left :class img :width 500px
[[./images/lambda.png][lambda examples]]

*** lambda triggers
lambda can be invoked via AWS SDK or triggered form other AWS services (e.g. s3, codecommit, api gateway) and 3rd party like datadog/one login

*** Pricing
first 1 million request/month free additional are .20 per million
400 000 GB seconds free per month

*** interface
#+attr_html: :alt  :align left :class img :width 500px
[[./images/lambda-interface.png][lambda interface]]

*** Default + limits
1000 lambda running concurrently (more requires AWS customer support)
/tmp dire max size 500 mb
timeout 15 mins maximize (more requires AWS fargate)
memory 128mb -> 3008mb at an increment of 64mb

*** Cold starts (largest negative)
AWS has reconfigured servers turned off that will start once the function is invoked. At this point the servers will turn on and the code will be copied over. The delay between the invocation and the execution is called =cold start=

With this in mind these cold starts can cause delays in user experience

Strategies such as pre-warming exists.

** Simple Queue Service
   - +[[Table of Content]]
*** Introduction
A fully managed queuing service that enables you to decouple and scale microservcie, distributed systems, and serverless apps

SQS is for Application integration. It connects osolated applications together by passing along messages to one another. SQS is pull based not push based

*** What is a queuing systems
message/events from sender and reciever
**** queueing (message broker)
deletes messages once they are consumed. not real-time (async). Not reactive
e.g. rabbitMQ, AWS SQS

**** Streaming (publish/subscribe)
real-time
multiple consumers can react to events. Events live in the stream for long periods of time so complex operations can be applied.
e.g AWS Kinesis, Apache Kafka

*** Use case
#+attr_html: :alt  :align left :class img :width 500px
[[./images/sqs.png][sqs messaging]]

*** Limits
**** message size
mesage can be between one 1 byte and 256kb.
AWS SQS extended client lib for java allows you to go up to 2GB. The message will be stored in s3

**** Message retention
default retention 4 days
min 60 seconds to max 14 days

*** Std queues
AWS SQS std queues allow you a nearly unlimited number of transactions per sec.
message will be delivered at least one.
more than one copy is possible
provides best effor ordering but can be delivered out of orders

*** FIFO (first in first out)
limited to 300 transactions per seconds

*** Visibility timeout
how to prevent another app from reading a message while another one is busy with taht message? avoid duplications.
Messages will be deleted from the queue after has processed ( before the expiry timeout expires ). If not processed before the visibility time-out. message will become visible again.
30 secs defaults, min 0 secs, max 12 hours.
Could lead to double messages

*** Short vs long polling
**** short polling (default)
returns message immediately, even if queue being polled is empty.
use it when you need a message right away.
**** Long polling
waits for until message arrives in the queue/long poll timeout expires. Reduces costs as it reduces the number of empty receives.
You want long plling in most cases

** Simple notification service (AWS SNS)
   - +[[Table of Content]]

*** Introduction
subscribe and send notification via sms, emails and mobile notifications.

#+attr_html: :alt  :align left :class img :width 500px
[[./images/sns.png][simple notification service]]
*** publish/subscribe
in this pattern the sender of messages (publisher) do not send thier messages directly to receivers. They instead send their messages to an event bus.
- Publishers have no knowledge who are their subscribers
- publishers push events to an SNS topics
- subscribers do not pull messages
- subscribers subscribe to SNS topics to have envets pushed to them
- messages are instead automatically andimmediately pused to subscribers
- messages and events are interchangeable

*** Topics
Topics allows you to group subscriptions together. Can be delivered to multiple protocoals at once e.g. sms, email, https, etc.
Topics can be encrypted with KMS

*** Subscriptions
A subscription can only subscribe to one protocol and one topic.
The following protocols:
- http/https create webhooks into web application
- email/email-json
- AWS SQS: place SNS message into SQS queue
- sms: sends text

** ElastiCache
   - +[[Table of Content]]
*** Introduction
managed caching service which runs redis or memcached.
cache is only available within the same VPC.

Stored in ram (fast but volatile)
#+attr_html: :alt  :align left :class img :width 500px
[[./images/elasticache.png][elasticache]]

*** Caching comparison
memcached prefered for caching HTML fragments. Simple key/value store.
redis: essentially a very fast k/v db. Has more features.

** High Availability
   - +[[Table of Content]]
*** Introduction
#+attr_html: :alt  :align left :class img :width 500px
[[./images/ha.png][high availability]]
tldr:
- use auto scaling groups
- run instances in multi-AZ with an elastic load balancer
- route traffic to another region using ruoter 53

*** horizontal scaling/vertical scalling
**** vertical scaling
easier to manage (get a bigger computer)
more vulnerable
**** horizontal scaling
More complex to manage
Can scale to infinity
** Elastic beanstalk (AWS heroku)
   - +[[Table of Content]]
*** Introduction
quickly deploy and manage web-apps on AWS (while not worrying about the infrastructure)
Not recommended for production.
Powered by cloudformation
Can run dockerized environments.

** API Gateway
   - +[[Table of Content]]
*** Introduction
Fully managed service to create, publish, maintain, monitor and secure APIs at any scale.
Can scale to thousands of concurrent API calls.
Can use version control for the endpoints

*** Configuration
resources: the urls you define e.g. /projects or /projects/-id-/edit
can have multiple method per resources
methods: GET/POST/UPDATE/DELETE
    when you create a method on a resource you need to  choose the integration type. most common lambda function
stages: stages are version of your APIs
invoke URL for each stage AWS provides you an url. possible to use custome domain
deploy API: for every change made you need to deploy it via the deploy API action

*** caching
available using Time to live (TTL).
API gateway respond by first looking at the cache
reduces numbers of call/latency time

*** CORS
allow restructed resources e.g. fonts to be requested from a different domain than the initial resource it came from
Cross-origin resource sharing (CORS) always enforced by the client

*** Same origin policy
concept in the application security model where a web browser permits scripts contained in a web page can access data in a second web page.
- prevents corss-site scripting attacks (XSS) type of injection attack
- inly works if both web pages have the same origin
- ignores tools like postman/curl
** Kinesis
   - +[[Table of Content]]
*** Introduction
scalable and durable =real-time= data streaming service to ingest, and analyze data in real-time from multiple sources
4 different types:
- data streams
- firehose delivery streams
- data analytics
- video analytics
**** data streams
#+attr_html: :alt  :align left :class img :width 500px
[[./images/kinesis.png][data streams]]
data only persists for 24 hours to 168 hrs
**** Firehose
only supports one consumer from a predefined list (e.g. s3)
data immediately disappears once its consumed

#+attr_html: :alt  :align left :class img :width 500px
[[./images/kinesis-firehose.png][firehose]]
**** video streams
ingest video + audio and output to ML/video processing services (AWS rekognition/AWS sageMaker/ tensorflow)
#+attr_html: :alt  :align left :class img :width 500px
[[./images/kinesis-video.png][video streams]]

**** data analytics
you can specify firehose or data streams as an input/output data that pass through =data analytics= is run through custom SQL you provide.
provides real time analytics of your data
** Storage Gateway
   - +[[Table of Content]]
*** Introduction
extending/backing up on-prem storage to the cloud (mixed infra)
Securely store your data to the AWS cloud. Software is available as a VM image.
*** types
- File Gateway (NFS)
- Volume Gateway (store copies of your hard drives in s3)
  * cached volumes
  * stored volumes
- Tape gateway (virtual tape lib)

*** File Gateway
files are stored as objects inside your s3 buckets. Can be managed as native s3 objects

*** Volume gateway
present your application with disk volumes using Internet Small Computer Systems Interface (iSCSI)
essentially stores your data in compressed snapshots
**** Stored volumes
primary data is stored locally, while async backing up
AWS helastic Block Store (EBS)
stored volumes can be between 1gb to 16 tb
#+attr_html: :alt  :align left :class img :width 500px
[[./images/stored-volumes.png][stored volumes]]
**** cached volumes
AWS s3 is the primaray data storage
#+attr_html: :alt  :align left :class img :width 500px
[[./images/cached-volumes.png][cached]]

**** tape gateway
AWS s3 is the primaray data storage
#+attr_html: :alt  :align left :class img :width 500px
[[./images/tape-gateway.png][tape]]
